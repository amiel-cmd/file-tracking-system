  ...
  try {
    switch (method) {
      ...
      case 'PUT': {
        ...
        // --- NEW: Check uniqueness only if document_number changed and not null ---
        ...
        const updatedSafeNumber = document_number
          ? `#${sanitize(document_number)}`
          : '(no manual number)';
        ...
        await logHistory(
          ...
          userId,
          'Document Updated',
          `Document details updated: "${sanitize(title)}" ${updatedSafeNumber}`,
        );
