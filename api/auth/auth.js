...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        if (url.endsWith('/register') && method === 'POST') {
            ...
            const validationErrors = validateRegistration({ full_name, username, email, password, confirm_password });
            if (validationErrors.length > 0) {
                return res.status(400).json({ success: false, error: validationErrors.join(', ') });
            }
            ...
            const usernameCheck = await pool.query('SELECT user_id FROM users WHERE username = $1', [sanitizedUsername]);
            if (usernameCheck.rows.length > 0) {
                return res.status(400).json({ success: false, error: 'Username already exists' });
            }
            const emailCheck = await pool.query('SELECT user_id FROM users WHERE email = $1', [sanitizedEmail]);
            if (emailCheck.rows.length > 0) {
                return res.status(400).json({ success: false, error: 'Email already registered' });
            }
            const hashedPassword = await bcrypt.hash(password, 10);
            const insertQuery = `INSERT INTO users (username, email, password, full_name, role, is_active) 
                                 VALUES ($1, $2, $3, $4, 'staff', 0) RETURNING user_id, username, email, full_name, role`;
