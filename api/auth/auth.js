...
const api = {
  async request(endpoint, options = {}) {
    ...
    console.log('API request to:', fullUrl, 'method:', options.method || 'GET');
...
const router = {
  ...
  showLogin() {
    ...
    document.getElementById('loginForm').addEventListener('submit', async (e) => {
      ...
      try {
        // POST to /api/auth with action in body
        ...
        router.showMessage(error.message, 'error');
      }
    });
  },
  ...
  showRegister() {
    ...
    document.getElementById('registerForm').addEventListener('submit', async (e) => {
      ...
      if (data.password !== data.confirm_password) {
        router.showMessage('Passwords do not match', 'error');
        return;
      }
      ...
      try {
        ...
        router.showMessage(result.message || 'Registration successful!', 'success');
